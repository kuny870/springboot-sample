...
public class DetectingProductService {
     ...
     */
    @Transactional
    public void regist(DetectingProduct detectingProduct) throws IOException {
...
//
//            f = new File(absolutePath + path + "\\" + fileOriginName);
//            detectingProduct.getFile().get(i).transferTo(f);
            ...
            dpi.setType("0");
            dpi.setProductImage(detectingProduct.getFile().get(i).getBytes());
     ...
     */
    @Transactional
    public void update(DetectingProduct detectingProduct) throws IOException  {
