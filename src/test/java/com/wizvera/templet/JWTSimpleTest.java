...
public class JWTSimpleTest {
    ...
    public void test_1() {
        String okta_token = Jwts.builder().addClaims(
                Map.of("name", "keonhee", "price", 3000)
                ).signWith(SignatureAlgorithm.HS256, "keonhee")
                .compact();
        System.out.println(okta_token);
        printToken(okta_token);

        Jws<Claims> tokenInfo = Jwts.parser().setSigningKey("keonhee").parseClaimsJws(okta_token);
    ...
    public void test_2() {
        ...
        byte[] SEC_KEY = DatatypeConverter.parseBase64Binary("keonhee");

        String oauth0_token = JWT.create().withClaim("name", "koenhee").withClaim("price", 3000)
    ...
    public void test_3() throws InterruptedException {
        ...
        final Algorithm AL = Algorithm.HMAC256("keonhee");
